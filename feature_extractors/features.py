...
from models.pointnet2_utils import interpolating_points # 点特征插值
from models.feature_fusion import FeatureFusionBlock # 特征融合
from models.models import Model # 整体模型类
# 8.14
...
class Features(torch.nn.Module):
    ...
    def __init__(self, args, image_size=224, f_coreset=0.1, coreset_eps=0.9):
        ...
        self.average = torch.nn.AvgPool2d(3, stride=1) # torch.nn.AvgPool2d(1, stride=1) # 二维平均池化
        self.resize = torch.nn.AdaptiveAvgPool2d((56, 56)) # 二维自适应平均池化
        self.resize2 = torch.nn.AdaptiveAvgPool2d((56, 56)) # 二维自适应平均池化
        ...
        if self.args.use_uff:
            ...
            incompatible = self.fusion.load_state_dict(ckpt, strict=False) # 读取训练好的权重

            print('[Fusion Block]', incompatible) # fusion模块是否完全匹配

        self.detect_fuser = linear_model.SGDOneClassSVM(random_state=42, nu=args.ocsvm_nu,  max_iter=args.ocsvm_maxiter) # 异常检测一类支持向量机（OCSVM）用于决策层融合（Decision Layer Fusion），输出异常分数anomaly detection
        self.seg_fuser = linear_model.SGDOneClassSVM(random_state=42, nu=args.ocsvm_nu,  max_iter=args.ocsvm_maxiter) # 异常分割图一类支持向量机（OCSVM）用于决策层融合（Decision Layer Fusion），输出异常分割图segmentation

        self.s_lib = []
        self.s_map_lib = []

    # Extract the desired feature maps using the backbone model 使用主干网络模型提取所需的特征图
    def __call__(self, rgb, xyz, missing_type=None, cp=None, mip=None, mpp=None):
        # print("features!!")
        rgb = rgb.to(self.device)
        xyz = xyz.to(self.device)
        with torch.no_grad():
            # print("features call!!")
            rgb_feature_maps, xyz_feature_maps, center, ori_idx, center_idx = self.deep_feature_extractor(rgb, xyz, missing_type)

        interpolate = True # 确定需要点特征插值
        if interpolate:
            interpolated_feature_maps = interpolating_points(xyz, center.permute(0,2,1), xyz_feature_maps).to("cpu")

        rgb_feature_maps = [fmap.to("cpu") for fmap in [rgb_feature_maps]] # 转化成列表
        xyz_feature_maps = [fmap.to("cpu") for fmap in [xyz_feature_maps]]  # 转化成列表
    ...
    # 样本添加到存储库中 在multiple_features中实现
    def add_sample_to_mem_bank(self, sample, missing_type):
        raise NotImplementedError

    # 运行控制资源集 在multiple_features中实现
    def run_coreset(self):
        raise NotImplementedError

    # 预测 在multiple_features中实现
    def predict(self, sample, mask, label, missing_type):
        raise NotImplementedError

    # 样本添加到之后特征融合的存储库中 在multiple_features中实现
    def add_sample_to_late_fusion_mem_bank(self, sample, missing_type):
        raise NotImplementedError

    # 点特征插值（Point Feature Interpolation）
    def interpolate_points(self, rgb, xyz, missing_type):
        with torch.no_grad():
            rgb_feature_maps, xyz_feature_maps, center, ori_idx, center_idx = self.deep_feature_extractor(rgb, xyz, missing_type)
        return xyz_feature_maps, center, xyz

    # 计算映射 在multiple_features中实现
    ...
    # 进行后续融合 决策层融合（Dicision Layer Fusion，DLF）
    def run_late_fusion(self):
        self.s_lib = torch.cat(self.s_lib, 0)
        self.s_map_lib = torch.cat(self.s_map_lib, 0)
        self.detect_fuser.fit(self.s_lib) # 异常检测OCSVM进行决策
        self.seg_fuser.fit(self.s_map_lib) # 异常分割OCSVM进行决策
