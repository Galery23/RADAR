...
def run_3d_ads(args):
    ...
    image_rocaucs_df = pd.DataFrame(METHOD_NAMES, columns=['Method']) # 存储图像级别准确率（I-AUROC）
    pixel_rocaucs_df = pd.DataFrame(METHOD_NAMES, columns=['Method']) # 存储像素级别准确率（P-AUROC）
    au_pros_df = pd.DataFrame(METHOD_NAMES, columns=['Method']) # 存储per-region overlap（AUPRO）

    # 按数据集中的类别处理
    for cls in classes:
        model = M3DM(args) # 初始化模型
        # model = M3DM(args, cls, 'train') # 初始化模型
        model.fit(cls) # 训练
        image_rocaucs, pixel_rocaucs, au_pros = model.evaluate(cls) # 测试评估
        # 得出评价指标的值
