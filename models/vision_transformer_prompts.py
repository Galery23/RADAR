...
from functools import partial
from tqdm import tqdm # PyTorch Image Models库是一个用于计算机视觉任务的Python库，提供了如ResNet、EfficientNet、ViT等常见深度学习模型架构的实现
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, prompts=None, learnt_p=False, prompt_type='input'):
        B, N, C = x.shape
        # 如果没有prompts（attention级别时后面几层不添加prompt）或者prompt类型为输入级别
        if prompts is None or prompt_type == 'input':
            # origin attention 原本的attention操作
            qkv = (
                self.qkv(x)
        ...
        # 如果prompt类型为注意力级别
            ...
            # prefix prompt tuning 前缀prompt调整
            P = prompts.size(1)
            qkv = (
                self.qkv(x)
                .reshape(B, N, 3, C)
            )   
            # 确定附加在键向量k和值向量v前面的prompt
            if learnt_p:
                P = P//2
                prompts_k = prompts[:,:P]
                prompts_v = prompts[:,P:]
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, prompts=None, learnt_p=False, prompt_type='input'):
        ...
        if prompts is not None:
            prompts = prompts.to(device) # 1111
        # 如果没有prompts或者prompt类型为输入级别
...
def _create_vision_transformer(variant, pretrained=False, distilled=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load(default_cfg['url']))
    #     load_pretrained(
    #         model,
    #         num_classes=num_classes,
...
@register_model
def vit_base_patch8_224_dino(pretrained=False):
    # print("vit_base_patch8_224_dino!!")
